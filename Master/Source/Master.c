...
tsKEY sKEY; //!< KEY 関連 @ingroup MASTER
...
PUBLIC uint8 cbToCoNet_u8HwInt(uint32 u32DeviceId, uint32 u32ItemBitmap) {
	...
	case E_AHI_DEVICE_TIMER0:
		// KEY の割り込み処理
		if (sKEY.execflg)
		vfPrintf(&sSerStream, "デバッグ counter:%d"LB, sKEY.counter);
		KEY_vInt(&sKEY);
		break;
 ...
 */PRIVATE void vInitHardware(int f_warm_start) {
	...
	// PWM1 を使用する。
	KEY_vInit(&sKEY, &sTimerPWM[0]); // sTimerPWM[0] 構造体は、sMML 構造体中にコピーされる。
...
static void vReceiveIoData(tsRxDataApp *pRx) {
	...
	for (i = 0, j = 1; i < 4; i++, j <<= 1) {
		if (u8ButtonChanged & j) {
			if ((u8ButtonState & j) && i == 0) {
				// ボタンの出力状態が Hi の場合のみ処理を行う。
				// Lo が継続している場合(ボタン長押し時)は無視。
				if (sAppData.sIOData_now.au8Output[i] == 0
						|| sAppData.sIOData_now.au8Output[i] == 0xFF) {
					KEY_vSet(&sKEY);
				}
			}
			else {
